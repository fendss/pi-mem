...
import { PiMemMemoryArenaAdapter } from "../src/benchmark/memoryarena-public/adapters/pimem-memory-runtime.js";
import {
...
async function temporaryPath(): Promise<{
  ...
  const directory = await mkdtemp(join(tmpdir(), "pimem-operation-audit-"));
...
async function failedAddTerminal(
  ...
  const chunks = new PiMemMemoryArenaAdapter({
    rawStore: store,
  ...
  try {
    await backend.initialize({ userId: mode, memorySystemName: "pimem" });
...
describe("MemoryArena Public durable operation audit", () => {
  ...
  it("keeps embedding usage when append completion fails after indexing", async () => {
    ...
    try {
      await backend.initialize({ userId: "completion", memorySystemName: "pimem" });
      await expect(backend.add({
        userId: "completion",
        memorySystemName: "pimem",
  ...
  it("records privacy-safe lifecycle, retrieval usage, and embedding deltas", async () => {
    ...
    await backend.initialize({ userId: "audit-user", memorySystemName: "pimem" });
    await backend.wrap({
      userId: "audit-user",
      memorySystemName: "pimem",
  ...
  it("keeps failed PiMem usage and a content-free trace summary", async () => {
    const paths = await temporaryPath();
    const meter = new RecordingEmbeddingMeter();
    const chunks = new MetricChunks(meter);
    const sink = new JsonlMemoryArenaOperationAuditSink(paths.audit);
    ...
    const piMemFailure = new PiMemRunError("SECRET model failure", {
      runId: "failed-run-id",
      ...
      usage: failureUsage,
    } as unknown as PiMemFailureDiagnostics);
    const retriever: MemoryArenaEvidenceRetriever = {
      retrieve: async () => {
        meter.record({
          calls: 2,
        ...
        throw new MemoryArenaPublicError({
          code: "upstream_unavailable",
          message: "PiMem retrieval is temporarily unavailable",
          httpStatus: 503,
          retryable: true,
          cause: piMemFailure,
        });
    ...
    await backend.initialize({ userId: "failure-user", memorySystemName: "pimem" });
    await backend.add({
      userId: "failure-user",
      memorySystemName: "pimem",
