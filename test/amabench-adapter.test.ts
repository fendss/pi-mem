...
describe("AMA-Bench v4 trusted adapter", () => {
  ...
  it("crosses the ordinary PiMem ingest and search ports without translation", async () => {
    const result = adaptAmaBenchV4(fixture());
    const directory = await mkdtemp(join(tmpdir(), "pimem-ama-ingest-"));
  ...
  it("parses JSONL strictly and rejects data without the pinned hash", async () => {
    ...
    const directory = await mkdtemp(join(tmpdir(), "pimem-ama-test-"));
    temporaryDirectories.push(directory);
