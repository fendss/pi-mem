...
import { runPiMem, type PiMemRuntimeStore } from "../src/evidence-agent/adapters/pi/run-agent.js";
...
describe("working-memory context policy", () => {
  ...
  it("performs search, read, repeated search, reread and finish without separate state calls or evidence loss", async () => {
    ...
    const output = await runPiMem({ store, operatorRegistry: createSearchOperatorRegistry(store), modelRuntime: runtime,
    ...
    await expect(runPiMem({ store, operatorRegistry: createSearchOperatorRegistry(store), modelRuntime: runtime,
