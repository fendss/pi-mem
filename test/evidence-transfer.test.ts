...
import { createPiMemTools } from "../src/evidence-agent/adapters/pi/tools.js";
...
function setup(source: MemoryRecord) {
  ...
  const tools = createPiMemTools({ store, ledger, scopeId: "scope", maxSearchCalls: 4,
...
describe("source fragments survive evidence transfer", () => {
  ...
  it("honors an explicit parent read even when a passage of that parent is selected", async () => {
