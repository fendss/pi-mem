...
export function qdrantRetrievalConfiguration(
  ...
  const collection: QdrantCollectionSpec = {
    ...
    hnsw: {
      m: environmentInteger(environment, "PIMEM_QDRANT_HNSW_M", 32, 256),
  ...
  return {
    ...
    client: new QdrantClient({
      baseUrl: requireEnvironmentVariable(environment, "PIMEM_QDRANT_URL"),
    ...
    verificationPollMs: environmentInteger(
      environment,
      "PIMEM_QDRANT_VERIFY_POLL_MS",
      1_000,
      60_000,
    ),
    verificationTimeoutMs: environmentInteger(
      environment,
      "PIMEM_QDRANT_VERIFY_TIMEOUT_MS",
      3_600_000,
      86_400_000,
    ),
  };
      ...
      if (
        ...
        throw new Error(
          `Qdrant generation ${config.generationId} does not cover requested ` +
            `scope ${scopeId}; use a new PIMEM_VECTOR_GENERATION_ID`,
        );
      }
    }
  }
