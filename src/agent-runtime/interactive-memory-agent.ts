...
export type InteractiveMemorySkill = "none" | "pimem-v0";

const ACTION_SKILL_PATH = fileURLToPath(
  new URL("../../.agents/skills/pimem-knowledge-action/SKILL.md", import.meta.url),
);
...
function activeSkillPrompt(skill: InteractiveMemorySkill): string {
  return skill === "none"
    ? ""
    : `<active_skill name="pimem-knowledge-action" version="${PIMEM_ACTION_SKILL_VERSION}">\n` +
...
function assertExternalDefinitions(
  ...
  for (const [index, definition] of definitions.entries()) {
    ...
    if (MEMORY_TOOL_NAMES.has(definition.name)) {
      throw new Error(`External tool conflicts with PiMem tool: ${definition.name}`);
    }
 ...
 * PiMem; caller-owned domain tools are returned to the environment and their
...
export class InteractiveMemoryAgentSession {
  ...
  constructor(private readonly options: InteractiveMemoryAgentOptions) {
    ...
    this.skill = options.skill ?? "pimem-v0";
    ...
    const memoryTools = createPiMemTools({
      store: options.store,
      ...
      if (event.type === "turn_start") {
        ...
        if (this.inputTurns > this.maxTurnsPerInput) {
          this.inputLimitError =
            `Interactive PiMem exceeded its per-input turn budget ` +
            `(${this.maxTurnsPerInput})`;
          this.agent.abort();
        }
        ...
        if (this.inputToolCalls > this.maxToolCallsPerInput) {
          this.inputLimitError =
            `Interactive PiMem exceeded its per-input tool-call budget ` +
            `(${this.maxToolCallsPerInput})`;
          this.agent.abort();
        }
  ...
  async turn(input: InteractiveAgentInput): Promise<InteractiveAgentOutput> {
    ...
    if (timedOut) {
      throw new Error(`Interactive PiMem input exceeded ${this.maxInputMs}ms`);
    }
    ...
    if (assistant === undefined) {
      throw new Error("Interactive PiMem stopped without an assistant message");
    }
    if (assistant.stopReason === "error" || assistant.stopReason === "aborted") {
      throw new Error(
        assistant.errorMessage ??
          `Interactive PiMem stopped with ${assistant.stopReason}`,
      );
    }
    ...
    return {
      ...
      audit: {
        sessionId: this.sessionId,
        harnessVersion: PIMEM_INTERACTIVE_HARNESS_VERSION,
        skill: {
          mode: this.skill,
          version: this.skill === "none" ? null : PIMEM_ACTION_SKILL_VERSION,
          hash: this.skill === "none" ? null : PIMEM_ACTION_SKILL_HASH,
        },
