...
export const PIMEM_HYBRID_BM25_OPTIONS: Readonly<Bm25Options> = {
  k1: 1.5,
  b: 0.75,
  epsilon: 0.25,
};
...
export function bm25Scores(
  ...
  options: Readonly<Bm25Options> = PIMEM_HYBRID_BM25_OPTIONS,
): number[] {
  if (documents.length === 0) return [];
  const tokenizedDocuments = documents.map(tokenizeForPiMemHybrid);
  ...
  const queryTokens = tokenizeForPiMemHybrid(query);
