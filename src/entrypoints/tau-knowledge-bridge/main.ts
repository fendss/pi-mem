...
function skillForBridge(raw: string | undefined): InteractiveMemorySkill {
  const skill = raw ?? "pimem-v0";
  if (skill !== "none" && skill !== "pimem-v0") {
    throw new Error(`Unknown interactive PiMem skill: ${skill}`);
  }
  return skill;
}
      ...
      try {
        ...
        if (session === undefined) {
          ...
          output({
            ...
            harness: PIMEM_INTERACTIVE_HARNESS_VERSION,
            skill: {
              mode: skill,
              version: skill === "none" ? null : PIMEM_ACTION_SKILL_VERSION,
              hash: skill === "none" ? null : PIMEM_ACTION_SKILL_HASH,
            },
...
main().catch((error: unknown) => {
  process.stderr.write(
    `PiMem tau-Knowledge bridge error: ${
