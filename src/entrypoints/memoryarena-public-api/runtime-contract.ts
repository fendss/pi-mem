...
export function skillHash(skill: PiMemSkill): string {
  if (skill === "pimem-v0") return PIMEM_SKILL_HASH;
  if (skill === "pimem-minimal") return PIMEM_MINIMAL_SKILL_HASH;
...
export function createMemoryArenaRuntimeIdentity(options: {
  ...
  const contract: MemoryArenaRuntimeContract = {
    schema_version: 1,
    source_identity: nonEmpty(options.sourceIdentity, "PIMEM_SOURCE_IDENTITY"),
    build_identity: nonEmpty(options.buildIdentity, "PIMEM_BUILD_IDENTITY"),
    skill: {
      id: options.skill,
    ...
    retrieval: {
      ...
      logical_model_id: nonEmpty(
        ...
        "PIMEM_LOGICAL_MODEL_ID",
      ),
      route_model_id: options.modelRuntime.modelId,
      protocol: nonEmpty(options.protocol, "PIMEM_RETRIEVAL_PROTOCOL"),
