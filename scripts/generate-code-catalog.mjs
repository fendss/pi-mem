...
const FILE_RESPONSIBILITIES = new Map(Object.entries({
  ...
  "src/benchmark/memoryagentbench/answer-contract.ts": "Builds MemoryAgentBench answer prompts from the benchmark question and PiMem-retrieved memory context.",
  "src/benchmark/memoryagentbench/dataset.ts": "Validates MemoryAgentBench records and maps their contexts and questions into the PiMem benchmark boundary.",
  ...
  "src/benchmark/memoryarena-public/model/failure-diagnostics.ts": "Defines privacy-safe structured diagnostics for failed MemoryArena PiMem retrieval runs.",
  ...
  "src/entrypoints/memoryarena-public-api/http-errors.ts": "Maps typed PiMem runtime failures to stable, privacy-safe MemoryArena HTTP errors.",
  ...
  "src/platform/sqlite/pimem-store.ts": "Provides the shared SQLite facade for memory persistence and injected retrieval indexes.",
  ...
  "src/retrieval/use-cases/compose-operator.ts": "Validates and builds bounded declarative operator graphs as CandidateSet transformations.",
...
const FUNCTION_PURPOSES = new Map(Object.entries({
  ...
  "src/evidence-agent/adapters/pi/run-agent.ts#runPiMem": "Runs one bounded evidence-agent session and returns its provenance-backed result.",
  ...
  "src/platform/sqlite/pimem-store.ts#MemoryStore.listScopeIds": "Lists immutable memory scope IDs in stable order for whole-corpus derived-index publication.",
  "src/platform/sqlite/pimem-store.ts#MemoryStore.getVectorGenerationScopeCount": "Returns one generation's durable vector count for a scope-level fail-closed retrieval check.",
