...
| --- | --- | --- |
| Memory identity | `memory.memory_system_name` | PiMem defaults to the literal `pimem` |
...
1. `POST /memory/initialize` with
   `{"user_id":"...","memory_system_name":"pimem"}`. Success is
   `{"status":"ok","user_id":"...","memory_system_name":"pimem"}`.
