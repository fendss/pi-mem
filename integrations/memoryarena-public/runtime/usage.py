...
def _pimem_tokens(raw: Mapping[str, Any]) -> Optional[dict[str, int | None]]:
    if not {"input", "output", "totalTokens"}.intersection(raw):
        return None
    if "input" not in raw or "output" not in raw:
        raise UsageNormalizationError("partial PiMem usage object")
    return {
...
def normalize_usage(
    ...
    *,
    ...
    """Normalize PiMem, Chat Completions, Responses, or canonical usage.
