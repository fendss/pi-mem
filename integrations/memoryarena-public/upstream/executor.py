...
_ENV = {
    ...
    "pimem_root": "MEMORYARENA_PIMEM_ROOT",
}
_MEMORY_URL_PATHS = {
...
class ProductionSettings:
    ...
    def from_environment(cls) -> "ProductionSettings":
        ...
        if not os.environ.get("PIMEM_DATA_DIR"):
            missing.append("PIMEM_DATA_DIR")
        ...
        return cls(
            ...
            pimem_root=Path(os.environ[_ENV["pimem_root"]]).resolve(),
            pimem_data_dir=Path(os.environ["PIMEM_DATA_DIR"]).resolve(),
...
def _verify_runtime_manifest(
    ...
    if (
        ...
        or not isinstance(models.get("pimem_retrieval"), str)
        or not models.get("pimem_retrieval")
        ...
        raise IntegrityError(
            ...
            "PiMem models, and provider proxy"
        )
    for environment_name, model_key in (
        ("MEMORYARENA_PIMEM_RETRIEVAL_MODEL", "pimem_retrieval"),
        ("MEMORYARENA_PIMEM_EMBEDDING_MODEL", "embedding_model"),
    ):
    ...
    if not (settings.pimem_root / ".git").exists():
        raise IntegrityError("MEMORYARENA_PIMEM_ROOT is not a PiMem Git worktree")
    try:
        top_level = Path(
            _run_git(settings.pimem_root, ["rev-parse", "--show-toplevel"])
        ).resolve()
        current_revision = _run_git(settings.pimem_root, ["rev-parse", "HEAD"])
        current_remote = _run_git(
            settings.pimem_root, ["remote", "get-url", "origin"]
        )
        ...
        raise IntegrityError(f"cannot verify PiMem source identity: {error}") from error
    if (
        top_level != settings.pimem_root
        or current_revision != pimem_source.get("revision")
        or _canonical_remote(current_remote)
        != _canonical_remote(str(pimem_source.get("repository", "")))
        or current_status
    ):
        raise IntegrityError(
...
def _validate_gateway_url(value: Any) -> str:
    if not isinstance(value, str):
        raise IntegrityError("effective PiMem URL is not a string")
    parsed = urlparse(value)
    if parsed.scheme not in {"http", "https"} or not parsed.hostname:
        raise IntegrityError("effective PiMem URL is invalid")
    if parsed.username or parsed.password:
        raise IntegrityError("effective PiMem URL contains credentials")
    return value.rstrip("/")
...
def _prepare_attempt_config(
    ...
    *,
    ...
    if not isinstance(memory, Mapping) or memory.get("memory_system_name") != "pimem":
        raise IntegrityError("effective config does not select the pimem backend")
...
def _pimem_model_identities(artifacts: AttemptHandle) -> tuple[str | None, str | None]:
    try:
    ...
    if not isinstance(models, Mapping):
        models = {}
    retrieval = models.get("pimem_retrieval")
...
def _read_pimem_operation_audits(
    settings: ProductionSettings,
    context: AttemptContext,
    artifacts: AttemptHandle,
) -> tuple[dict[str, Any], ...]:
    path = settings.pimem_data_dir / "operation-audits.jsonl"
    if not path.is_file():
        raise _incomplete_provider_error(
            f"PiMem durable operation audit is missing: {path}", stage="pimem-memory"
        )
    ...
    try:
        ...
        raise _incomplete_provider_error(
            f"PiMem durable operation audit is unreadable: {error}",
            stage="pimem-memory",
        ) from error
    if not records:
        raise _incomplete_provider_error(
            "PiMem durable operation audit has no events for attempt scope",
            stage="pimem-memory",
        )
    ...
    for record in records:
        ...
        if not isinstance(operation_id, str) or not operation_id:
            raise IntegrityError("PiMem operation audit lacks operation_id")
    ...
    for operation_id, lifecycle in grouped.items():
        if len(lifecycle) != 2 or lifecycle[0].get("phase") != "start" or lifecycle[1].get(
            ...
            raise _incomplete_provider_error(
                f"PiMem operation audit lifecycle incomplete: {operation_id}",
                stage="pimem-memory",
            )
        if lifecycle[0].get("operation") != lifecycle[1].get("operation"):
            raise IntegrityError("PiMem operation audit operation changed within lifecycle")
        terminals.append(lifecycle[1])
    write_json_atomic(
        context.attempt_dir / "upstream" / "pimem-operation-audit.json",
        {
            "schema_version": 1,
            "memory_scope": context.memory_scope,
    ...
    retrieval_model, embedding_model = _pimem_model_identities(artifacts)
    ...
    for terminal in terminals:
        ...
        if isinstance(retrieval, Mapping) and isinstance(retrieval.get("usage"), Mapping):
            if retrieval_model is None:
                raise IntegrityError(
                    "PiMem retrieval usage is observable but retrieval model identity is not locked"
                )
            usage = retrieval["usage"]
            artifacts.append_event(
                component="pimem-retrieval",
                kind="raw-usage-observed",
                payload={
        ...
        if isinstance(delta, Mapping):
            ...
            if input_tokens is not None:
                if embedding_model is None:
                    raise IntegrityError(
                        "PiMem embedding usage is observable but embedding model identity is not locked"
                    )
                raw_embedding_usage = {
...
def _capture_pimem_wrap_audits(
    settings: ProductionSettings,
    context: AttemptContext,
    artifacts: AttemptHandle,
    *,
    ...
    """Copy this attempt's full PiMem retrieval trajectories into its artifacts."""

    source = settings.pimem_data_dir / "wrap-audits.jsonl"
    if not source.is_file():
        raise _incomplete_provider_error(
            f"PiMem raw wrap audit is missing: {source}", stage="pimem-memory"
        )
    prefix = context.memory_scope + "::"
    selected: list[bytes] = []
    retrieval_model, _ = _pimem_model_identities(artifacts)
    try:
            ...
            for line_number, raw_line in enumerate(handle, start=1):
                ...
                if isinstance(retrieval, Mapping):
                    ...
                    if retrieval_model is None or model_id != retrieval_model:
                        raise IntegrityError(
                            "PiMem raw trajectory retrieval model differs from the "
                            "SHA-locked run manifest"
                        )
                selected.append(raw_line.rstrip(b"\r\n") + b"\n")
    except (OSError, ValueError, json.JSONDecodeError) as error:
        raise _incomplete_provider_error(
            f"PiMem raw wrap audit is unreadable: {error}", stage="pimem-memory"
        ) from error
    ...
    if len(selected) != expected_successes:
        raise _incomplete_provider_error(
            "PiMem raw trajectory count differs from successful wrap lifecycle",
            stage="pimem-memory",
        )
    target = context.attempt_dir / "upstream" / "pimem-wrap-audits.jsonl"
    ...
    artifacts.append_event(
        ...
        payload={
            "path": "upstream/pimem-wrap-audits.jsonl",
...
def memoryarena_executor(
    ...
    try:
        ...
        wrap_audit_path, wrap_audit_count = _capture_pimem_wrap_audits(
            settings,
            context,
            artifacts,
        ...
        # partial official usage; otherwise absence of durable PiMem lifecycle
    ...
    if durable_operations != proxy_operations:
        raise _incomplete_provider_error(
            "PiMem durable lifecycle differs from attempt-local proxy audit",
            stage="pimem-memory",
        )
    ...
    if durable_statuses != proxy_statuses:
        raise _incomplete_provider_error(
            "PiMem durable/proxy terminal statuses differ", stage="pimem-memory"
        )
    try:
        ...
        raise ProviderRequestError(
            str(error),
            status_code=status,
            provider_code="pimem_lifecycle_incomplete",
            retry_after=(failed.retry_after if failed else None),
            stage="pimem-memory",
        ) from error
    artifacts.append_event(
        component="pimem-memory",
    ...
    return TaskExecutionResult(
        ...
        metadata={
            ...
            "pimem_wrap_audit_sha256": file_sha256(wrap_audit_path),
