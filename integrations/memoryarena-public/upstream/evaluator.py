...
def _validate_production_attempt(
    ...
    expected_artifacts = {
        ...
        "pimem_wrap_audit_sha256": attempt_dir / "upstream/pimem-wrap-audits.jsonl",
    }
    ...
    trajectory_path = attempt_dir / "upstream/pimem-wrap-audits.jsonl"
    if (
        not isinstance(trajectory, Mapping)
        or trajectory.get("path") != "upstream/pimem-wrap-audits.jsonl"
        ...
        raise EvaluatorMaterializationError(
            f"accepted raw PiMem trajectory binding mismatch: {task.task_key}"
        )
    try:
...
def _materialize_formal(
    ...
    candidates = list(
        (attempt / "upstream" / "official").glob(f"pimem/{paper}/result.jsonl")
    )
    ...
    destination = target / task.domain / "pimem" / paper / "result.jsonl"
    return [_copy(source, destination)]
...
def _validate_formal_evaluator_input(
    ...
    pimem_root = work_root / "pimem"
    actual_papers = {
        path.name for path in pimem_root.iterdir() if path.is_dir() and not path.is_symlink()
    } if pimem_root.is_dir() else set()
    if (
    ...
    for paper, expected_count in expected_papers.items():
        path = pimem_root / paper / "result.jsonl"
        if path.is_symlink() or not path.is_file():
...
def _validate_travel_csv(path: Path, model: str) -> None:
    ...
    if (
        ...
        or rows[0].get("memory_system") != "pimem"
    ...
    *,
    ...
    stages.append(_run_stage(
        ...
        [python, str(travel["evaluator"]), "--submission_path", str(submission),
         "--model_name", travel_model, "--memory_system", "pimem",
    ...
    for suite in ("formal_reasoning_math", "formal_reasoning_phys"):
        ...
        result = work_root / "pimem/all_results.json"
