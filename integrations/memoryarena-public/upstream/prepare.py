...
def _git(root: Path, *args: str) -> str:
    ...
    if process.returncode != 0:
        raise UpstreamContractError(
            f"cannot identify clean PiMem source ({' '.join(args)}): "
            f"{process.stderr.strip()[:1000]}"
        )
    return process.stdout.strip()


def pimem_source_identity(root: Path) -> dict[str, object]:
    source = root.resolve()
    if Path(_git(source, "rev-parse", "--show-toplevel")).resolve() != source:
        raise UpstreamContractError("--pimem-root must be the PiMem Git worktree root")
    revision = _git(source, "rev-parse", "HEAD")
    if len(revision) != 40 or any(character not in "0123456789abcdef" for character in revision):
        raise UpstreamContractError("PiMem HEAD is not a full lowercase Git commit")
    if _git(source, "status", "--porcelain=v1", "--untracked-files=all"):
        raise UpstreamContractError(
            "PiMem source must be fully clean, including untracked files, before a production run"
        )
    remote = _git(source, "remote", "get-url", "origin")
    if not remote:
        raise UpstreamContractError("PiMem source has no origin remote")
    parsed_remote = urlsplit(remote)
    if parsed_remote.scheme in {"http", "https"}:
        if parsed_remote.username or parsed_remote.password:
            raise UpstreamContractError("PiMem origin URL must not contain credentials")
        if parsed_remote.query or parsed_remote.fragment:
            raise UpstreamContractError("PiMem origin URL must not contain query/fragment data")
        remote = urlunsplit(
            (
...
def prepare_source_run_manifest(
    *,
    ...
    if not run_id or not retrieval_model or not embedding_model:
        raise UpstreamContractError("run id and PiMem model identities must be non-empty")
    ...
    return locked_manifest_document({
        ...
        "models": {
            "pimem_retrieval": retrieval_model,
