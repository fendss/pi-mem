...
class GoldLeakGuard:
    ...
    def check(self, operation: str, payload: Mapping[str, Any]) -> None:
        ...
        if not isinstance(text, str):
            raise UpstreamExecutionError(f"PiMem {operation} payload has no string {field}")
        ...
        if any(marker in normalized for marker in forbidden_markers):
            raise UpstreamExecutionError(
                f"explicit reference-answer payload rejected before PiMem {operation}"
            )
    ...
    def __init__(
        ...
        *,
        ...
        if parsed.scheme not in {"http", "https"} or not parsed.hostname:
            raise UpstreamExecutionError("PiMem upstream must be an HTTP(S) URL")
        if parsed.username or parsed.password or parsed.query or parsed.fragment:
            raise UpstreamExecutionError("PiMem upstream URL must not contain credentials/query")
        ...
        # shell proxy settings must not silently reroute localhost PiMem calls.
            ...
            def do_POST(self) -> None:  # noqa: N802 - stdlib callback
                ...
                try:
                    ...
                    if not isinstance(payload, dict) or set(payload) != keys:
                        raise UpstreamExecutionError(
                            f"official PiMem {operation} JSON keys changed"
                        )
                    ...
                    try:
                        ...
                        retryable_header = response.headers.get("x-pimem-retryable")
                        ...
                        retryable_header = error.headers.get("x-pimem-retryable")
                    event = AuditEvent(
                    ...
                    if retryable_header is not None:
                        self.send_header("x-pimem-retryable", retryable_header)
...
def validate_lifecycle(
    ...
    *,
    ...
    if errors:
        first = errors[0]
        raise UpstreamExecutionError(
            f"PiMem {first.operation} failed with {first.status_code}: {first.error}"
        )
    ...
    if counts != expected or len(users) != 1:
        raise UpstreamExecutionError(
            f"PiMem lifecycle mismatch for {suite}: expected {expected}, got {counts}, users={len(users)}"
        )
