...
class UpstreamEvaluatorTests(unittest.TestCase):
    ...
    def test_formal_stage_requires_exact_release_rows_and_metric_coverage(self):
        ...
        for index, count in enumerate(counts):
            ...
            result = work_root / "pimem" / paper / "result.jsonl"
            result.parent.mkdir(parents=True)
            result.write_text(
                "".join(
        ...
        output = work_root / "pimem/all_results.json"
        output.write_text(
            json.dumps(
                {
        ...
        first_result = work_root / "pimem/paper-00/result.jsonl"
