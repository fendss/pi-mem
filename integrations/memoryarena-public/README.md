...
This directory connects PiMem to the official public MemoryArena release without
...
Run this from the PiMem repository root. The checkout is intentionally placed
...
The default PiMem gateway identity is `pimem` at
...
The commands below are intended to be copied from a clean PiMem Git worktree.
...
PiMem worktree: production preparation rejects tracked changes and untracked
files and binds the full clean PiMem `HEAD` to the source run manifest.

```bash
export MA_PIMEM_ROOT="$PWD"
export MA_INTEGRATION="$MA_PIMEM_ROOT/integrations/memoryarena-public"
export MA_WORK="$MA_PIMEM_ROOT/../memoryarena-work/run-001"
...
names = (
    "MA_PIMEM_ROOT", "MA_INTEGRATION", "MA_WORK", "MA_CHECKOUT", "MA_SETUP",
...
The generated environment file is outside the PiMem worktree and deliberately
contains no API keys. At the start of **every additional shell**, open that
shell in the PiMem repository and restore the shared paths with:

```bash
export MA_PIMEM_ROOT="$(git rev-parse --show-toplevel)"
source "$MA_PIMEM_ROOT/../memoryarena-work/run-001/memoryarena-public.env"
```
...
whose response omits usage) and preserve `Retry-After`. It is not the PiMem
...
8001 endpoint fields. The PiMem wrapper does not launch or patch this server.
...
### 4. Start the dedicated PiMem memory gateway

Build PiMem, then start this command in a separate shell. The data directory is
...
cd "$MA_PIMEM_ROOT"
npm run build
export PIMEM_DATA_DIR="$MA_WORK/pimem-data"
...
export OPENAI_API_BASE="$MA_PROVIDER_PROXY"
export OPENAI_BASE_URL="$MA_PROVIDER_PROXY"
# Also set OPENAI_API_KEY, PIMEM_EMBEDDING_BASE_URL, and
...
are used only by local integrity checks and are rejected from all PiMem HTTP
request bodies.
...
export MEMORYARENA_PIMEM_ROOT="$MA_PIMEM_ROOT"
export PIMEM_DATA_DIR="$MA_WORK/pimem-data"
export OPENAI_BASE_URL="$MA_PROVIDER_PROXY"
export OPENAI_API_BASE="$MA_PROVIDER_PROXY"
...
fallbacks detected from the PiMem lifecycle, fail the whole task group. A retry
...
- PiMem retrieval and embedding usage comes from durable operation audits;
