...
This integration runs PiMem on the fixed 50-question LongMemEval-S sample used
by STITCH and ReFind. It does not copy ReFind's search implementation: PiMem
...
PiMem is capped at four search executions per question, matching ReFind's
...
`run_s50.sh` expects a pre-indexed, 50-question PiMem data directory. It uses
...
prompt hash, response cache, and output directory; it never replaces PiMem's
