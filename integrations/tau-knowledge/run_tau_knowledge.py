#!/usr/bin/env python3
"""Run PiMem inside the pinned official tau-Knowledge environment."""
...
from pimem_tau_agent import (
    BRIDGE_ARGS_ENV,
    BRIDGE_RESPONSE_TIMEOUT_ENV,
    create_pimem_tau_agent,
)
...
DEFAULT_PIMEM_MAX_INPUT_MS = 900_000
DEFAULT_PIMEM_MAX_TURNS_PER_INPUT = 64
DEFAULT_PIMEM_MAX_TOOL_CALLS_PER_INPUT = 128
...
def assert_recipe(args: argparse.Namespace) -> None:
    ...
    if args.condition == "pimem" and args.run_kind == "formal":
        for flag, value in [
            ...
            if not value:
                mismatches.append(f"formal PiMem runs require explicit {flag}")
    if mismatches:
...
def integration_identity() -> dict[str, str]:
    ...
    return {
        ...
        "agent_adapter_sha256": hashlib.sha256(
            (root / "pimem_tau_agent.py").read_bytes()
        ).hexdigest(),
    ...
    *,
    ...
    if not allow_infra_resume:
        raise SystemExit(
            ...
            "benchmark semantics and PiMem source are unchanged."
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--condition",
        choices=["official-alltools", "pimem"],
        default="pimem",
        help="Official AllTools calibration or the PiMem custom condition",
    )
    parser.add_argument(
        "--run-kind",
    ...
    parser.add_argument(
        "--pimem-max-input-ms", type=int, default=DEFAULT_PIMEM_MAX_INPUT_MS
    )
    parser.add_argument(
        "--pimem-max-turns-per-input",
        type=int,
        default=DEFAULT_PIMEM_MAX_TURNS_PER_INPUT,
    )
    parser.add_argument(
        "--pimem-max-tool-calls-per-input",
        type=int,
        default=DEFAULT_PIMEM_MAX_TOOL_CALLS_PER_INPUT,
    )
    parser.add_argument(
    ...
    parser.add_argument("--skill", choices=["none", "pimem-v0"], default="pimem-v0")
...
def main() -> None:
    ...
    if not 1_000 <= args.pimem_max_input_ms <= 1_800_000:
        raise SystemExit("--pimem-max-input-ms must be between 1000 and 1800000")
    if not 1 <= args.pimem_max_turns_per_input <= 128:
        raise SystemExit("--pimem-max-turns-per-input must be between 1 and 128")
    if not 1 <= args.pimem_max_tool_calls_per_input <= 256:
        raise SystemExit(
            "--pimem-max-tool-calls-per-input must be between 1 and 256"
        )
    if not 1 <= args.bridge_response_timeout <= 1800:
        raise SystemExit("--bridge-response-timeout must be between 1 and 1800")
    if args.condition == "pimem" and (
        args.bridge_response_timeout * 1000 < args.pimem_max_input_ms + 30_000
    ):
        raise SystemExit(
            "--bridge-response-timeout must exceed --pimem-max-input-ms by at least 30 seconds"
        )
    ...
    pimem_source = source_identity(pimem_root, args.allow_dirty)
    bridge = pimem_root / "dist/tau-knowledge-bridge.js"
    data_manifest_path = data_dir / "tau-knowledge-manifest.json"
    if not bridge.is_file():
        raise SystemExit("PiMem bridge is not built; run npm run build first")
    ...
    if args.condition == "pimem":
        bridge_args = [
            ...
            args.skill,
            "--max-input-ms",
            str(args.pimem_max_input_ms),
            "--max-turns-per-input",
            str(args.pimem_max_turns_per_input),
            "--max-tool-calls-per-input",
            str(args.pimem_max_tool_calls_per_input),
        ]
    ...
    for flag, value in [
        ...
        if value and args.condition == "pimem":
    ...
    # PiMem supplies the retrieval tools inside its own Agent runtime.
    ...
    if args.condition == "pimem":
        RETRIEVAL_VARIANTS["pimem"] = RetrievalVariant(
            name="pimem",
            prompt_template=PROMPTS_DIR / "no_knowledge.md",
            build_prompt=standard_prompt,
        )
        registry.register_agent_factory(create_pimem_tau_agent, "pimem")
    ...
    manifest = {
        ...
        "pimem_source": pimem_source,
        "integration_source": integration_identity(),
        "provider_endpoints": {
            "tau_openai": tau_openai_endpoint,
            "pimem_agent": pimem_endpoint if args.condition == "pimem" else None,
            "pimem_embedding": (
                embedding_endpoint if args.condition == "pimem" else None
            ),
        },
        "execution_contract": {
            ...
            "pimem_max_input_ms": (
                args.pimem_max_input_ms if args.condition == "pimem" else None
            ),
            "pimem_max_turns_per_input": (
                args.pimem_max_turns_per_input
                if args.condition == "pimem"
                else None
            ),
            "pimem_max_tool_calls_per_input": (
                args.pimem_max_tool_calls_per_input
                if args.condition == "pimem"
                else None
            ),
        ...
        "config": {
            ...
            "skill": args.skill if args.condition == "pimem" else None,
            "operators": operators,
        ...
        config = TextRunConfig(
            **shared_config,
            ...
            llm_agent=(
                ...
                or f"pimem/{args.model}/{args.thinking_level}"
            ),
            llm_args_agent={"temperature": OFFICIAL_TEMPERATURE},
            user="user_simulator",
            retrieval_config="pimem",
        )
