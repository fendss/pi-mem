...
This directory connects PiMem to the pinned official `tau2-bench` runner. It
...
cross the bridge. The custom `pimem` retrieval variant uses the official
`no_knowledge` prompt because PiMem supplies the only knowledge-retrieval tools.

## Files

- `pimem_tau_agent.py` implements the official `HalfDuplexAgent` interface and
...
and embedding endpoints but never API keys. PiMem's per-input deadline is
...
The interactive PiMem runtime itself permits 64 internal Agent turns and 128
...
PiMem source to match exactly. Concurrency may be reduced during recovery
...
- Adaptive routing: `--skill pimem-v0`
...
The `official-alltools` condition is a standard-scaffold calibration. PiMem
