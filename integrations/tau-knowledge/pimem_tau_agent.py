"""Thin tau2 HalfDuplexAgent adapter for the PiMem JSONL bridge.
...
BRIDGE_ARGS_ENV = "PIMEM_TAU_BRIDGE_ARGS"
BRIDGE_RESPONSE_TIMEOUT_ENV = "PIMEM_TAU_BRIDGE_RESPONSE_TIMEOUT_SECONDS"
...
class PiMemBridgeError(RuntimeError):
    pass
...
class PiMemBridge:
    def __init__(
        ...
        if not serialized_args:
            raise PiMemBridgeError(f"{BRIDGE_ARGS_ENV} is not configured")
        try:
            args = json.loads(serialized_args)
        except json.JSONDecodeError as error:
            raise PiMemBridgeError(f"{BRIDGE_ARGS_ENV} is invalid JSON") from error
        if not isinstance(args, list) or not args or not all(
            isinstance(item, str) and item for item in args
        ):
            raise PiMemBridgeError(f"{BRIDGE_ARGS_ENV} must be a JSON string array")
        ...
        for tool in tools:
            ...
            if not isinstance(function, dict):
                raise PiMemBridgeError(f"tau2 tool {tool.name} has no function schema")
            parameters = function.get("parameters")
            if not isinstance(parameters, dict):
                raise PiMemBridgeError(f"tau2 tool {tool.name} has no parameter schema")
            definitions.append(
                {
                    "name": tool.name,
        ...
        if ready.get("type") != "ready":
            raise PiMemBridgeError("PiMem bridge did not acknowledge initialization")
        self.identity = ready

    def request(self, payload: dict[str, Any]) -> dict[str, Any]:
        if self._process.stdin is None or self._process.stdout is None:
            raise PiMemBridgeError("PiMem bridge pipes are unavailable")
        if self._process.poll() is not None:
            raise PiMemBridgeError(
                f"PiMem bridge exited with code {self._process.returncode}"
            )
        ...
        if not readable:
            self.close()
            raise PiMemBridgeError(
                "PiMem bridge response exceeded "
                f"{self._response_timeout:g} seconds"
            )
        line = self._process.stdout.readline()
        if not line:
            self.close()
            raise PiMemBridgeError("PiMem bridge closed without a response")
        try:
            response = json.loads(line)
        except json.JSONDecodeError as error:
            raise PiMemBridgeError("PiMem bridge returned invalid JSON") from error
        if not isinstance(response, dict):
            raise PiMemBridgeError("PiMem bridge response must be an object")
        if response.get("type") == "error":
            message = str(response.get("message") or "bridge error")
            self.close()
            raise PiMemBridgeError(message)
        return response
    ...
    def assistant_for(self, response: dict[str, Any]) -> AssistantMessage:
        if response.get("type") != "assistant":
            raise PiMemBridgeError("PiMem bridge response is not an assistant turn")
        raw_calls = response.get("toolCalls")
        if not isinstance(raw_calls, list):
            raise PiMemBridgeError("PiMem bridge assistant has no tool-call list")
        tool_calls: list[ToolCall] = []
        next_pending: dict[str, str] = {}
        for index, raw_call in enumerate(raw_calls):
            if not isinstance(raw_call, dict):
                raise PiMemBridgeError(f"Invalid bridge tool call at index {index}")
        ...
        if content is not None and not isinstance(content, str):
            raise PiMemBridgeError("PiMem bridge assistant content is invalid")
        ...
        return AssistantMessage(
            ...
            cost=cost,
            raw_data={
                "pimem": response.get("audit"),
                "model": response.get("model"),
                "pimem_runtime": self.identity,
            },
        )
...
class PiMemTauAgent(HalfDuplexAgent[PiMemTauState]):
    def __init__(self, tools: list[Tool], domain_policy: str):
        super().__init__(tools=tools, domain_policy=domain_policy)
        self._bridge: Optional[PiMemBridge] = None
    ...
    def get_init_state(
        ...
        if self._bridge is not None:
            raise PiMemBridgeError("PiMem tau agent was initialized more than once")
        self._bridge = PiMemBridge(
            self.tools,
            self.domain_policy,
            DEFAULT_FIRST_AGENT_MESSAGE,
        )
        return PiMemTauState()

    def generate_next_message(
        self, message: Message, state: PiMemTauState
    ) -> tuple[AssistantMessage, PiMemTauState]:
        if self._bridge is None:
            raise PiMemBridgeError("PiMem tau agent is not initialized")
    ...
    def stop(
        ...
        state: Optional[PiMemTauState] = None,
    ) -> None:
        if self._bridge is not None:
            self._bridge.close()


def create_pimem_tau_agent(tools, domain_policy, **_kwargs):
    return PiMemTauAgent(tools=tools, domain_policy=domain_policy)
